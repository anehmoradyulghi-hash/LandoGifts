 ...
 * Table creation lives in migrations/008_league.sql now.
 * ========================================================================= */

const LEAGUES = ['bronze', 'silver', 'gold'];
const LEAGUE_LABELS = { bronze: '🥉 Bronze', silver: '🥈 Silver', gold: '🥇 Gold' };

export async function getLeagueConfig() { return await db.prepare('SELECT * FROM league_config WHERE id = 1').get(); }
export async function setLeagueConfig(c) {
  await db.prepare(`
...
async function getLeagueState() { return await db.prepare('SELECT * FROM league_state WHERE id = 1').get(); }

export async function getOrCreateUserLeague(tgId) {
  await db.prepare('INSERT INTO user_league (tg_id) VALUES (?) ON CONFLICT (tg_id) DO NOTHING').run(tgId);
  return await db.prepare('SELECT * FROM user_league WHERE tg_id = ?').get(tgId);
}
function scoreOf(row) { return row.weekly_wins * 3 - row.weekly_losses; }

export async function getUserLeague(tgId) {
  return (await getOrCreateUserLeague(tgId)).league;
}

export async function recordLeagueResult(tgId, won) {
  const cfg = await getLeagueConfig();
  if (!cfg.enabled) return;
  await getOrCreateUserLeague(tgId);
  await db.prepare(`UPDATE user_league SET ${won ? 'weekly_wins = weekly_wins + 1' : 'weekly_losses = weekly_losses + 1'} WHERE tg_id = ?`).run(tgId);
}

export async function getUserLeagueInfo(tgId) {
  const row = await getOrCreateUserLeague(tgId);
...
export async function getLeagueLeaderboard(league, limit = 10) {
...
export async function pickQueueOpponentInLeague(league, excludeTgId) {
...
async function resolveWeeklyLeagues() {
  const cfg = await getLeagueConfig();
  const order = LEAGUES; // bronze -> silver -> gold
  const snapshot = {};
  for (const league of order) {
    snapshot[league] = await db.prepare(`
  ...
  const tx = db.transaction(async () => {
    for (const mv of moves) await db.prepare('UPDATE user_league SET league = ? WHERE tg_id = ?').run(mv.newLeague, mv.tg_id);
    await db.prepare('UPDATE user_league SET weekly_wins = 0, weekly_losses = 0').run();
    await db.prepare(`UPDATE league_state SET period_started_at = now_text() WHERE id = 1`).run();
  });
  await tx();
}

export async function checkAutoResetLeague() {
  const cfg = await getLeagueConfig();
  if (!cfg.enabled) return;
  const state = await getLeagueState();
  const startedAt = new Date(state.period_started_at.replace(' ', 'T') + 'Z').getTime();
  if (Date.now() >= startedAt + cfg.reset_days * 24 * 60 * 60 * 1000) await resolveWeeklyLeagues();
}

 ...
 * Table creation lives in migrations/009_wheel.sql now.
 * ========================================================================= */

export async function getWheelConfig() { return await db.prepare('SELECT * FROM wheel_config WHERE id = 1').get(); }
export async function setWheelConfig({ enabled, cooldown_hours, require_purchase }) {
  await db.prepare(`UPDATE wheel_config SET enabled=?, cooldown_hours=?, require_purchase=? WHERE id=1`)
    .run(enabled ? 1 : 0, Number(cooldown_hours) || 24, require_purchase ? 1 : 0);
}

export async function listWheelSlots(onlyActive = false) {
  return onlyActive
    ? await db.prepare('SELECT * FROM wheel_slots WHERE active = 1 ORDER BY id ASC').all()
    : await db.prepare('SELECT * FROM wheel_slots ORDER BY id ASC').all();
}
export async function upsertWheelSlot(s) {
  if (s.id) {
    await db.prepare(`
...
export async function deleteWheelSlot(id) { await db.prepare('DELETE FROM wheel_slots WHERE id = ?').run(id); }

export async function getLastSpin(tgId) {
  return await db.prepare('SELECT * FROM wheel_spins WHERE tg_id = ? ORDER BY spun_at DESC LIMIT 1').get(tgId);
}
export async function getWheelStatus(tgId) {
  const cfg = await getWheelConfig();
  if (!cfg.enabled) return { enabled: false };
  const last = await getLastSpin(tgId);
  let nextSpinAt = null;
  if (last) {
  ...
  if (cfg.require_purchase) {
    const orderCount = (await db.prepare('SELECT COUNT(*) c FROM orders WHERE tg_id = ?').get(tgId)).c;
...
export async function spinWheel(tgId) {
  const status = await getWheelStatus(tgId);
  ...
  const allSlots = await listWheelSlots(true);
  const slots = [];
  for (const s of allSlots) {
    if (s.type !== 'card' || await db.prepare('SELECT 1 FROM game_cards WHERE id = ?').get(s.card_id)) {
      slots.push(s);
    }
  }
  ...
  const tx = db.transaction(async () => {
    if (chosen.type === 'toman' && chosen.amount_toman > 0) {
      await adjustToman(tgId, chosen.amount_toman, `Wheel of fortune prize: ${chosen.label}`);
    } else if (chosen.type === 'card' && chosen.card_id) {
      await grantCardInstance(tgId, chosen.card_id);
    ...
    await db.prepare('INSERT INTO wheel_spins (tg_id, slot_id, result_label) VALUES (?,?,?)').run(tgId, chosen.id, chosen.label);
  });
  await tx();
...
export async function getWheelHistory(tgId, limit = 10) {
  return await db.prepare('SELECT * FROM wheel_spins WHERE tg_id = ? ORDER BY spun_at DESC LIMIT ?').all(tgId, limit);
}

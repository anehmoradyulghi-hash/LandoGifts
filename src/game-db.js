...
/* =========================================================================
 * USER CARDS — خرید، دیدن مجموعه، ارتقا (پولی یا با ادغام دو کارت مشابه)
 * ========================================================================= */
export function getUserCards(tgId) {
  return db.prepare(`
    SELECT uc.id, uc.card_id, uc.level, uc.created_at,
      c.name, c.image_url, c.level_images, c.rarity, c.base_power, c.max_level, c.category_id, c.edition
    FROM user_cards uc JOIN game_cards c ON c.id = uc.card_id
    WHERE uc.tg_id = ? ORDER BY uc.id DESC
  `).all(tgId).map(row => ({ ...row, power: computeCardPower(row.base_power, row.level), image: getCardImageForLevel(row, row.level) }));
}
export function getUserCard(tgId, userCardId) {
  const row = db.prepare(`
    SELECT uc.*, c.name, c.image_url, c.level_images, c.rarity, c.base_power, c.max_level, c.price_toman, c.category_id, c.edition
    FROM user_cards uc JOIN game_cards c ON c.id = uc.card_id
    WHERE uc.id = ? AND uc.tg_id = ?
  `).get(userCardId, tgId);
  return row ? { ...row, power: computeCardPower(row.base_power, row.level), image: getCardImageForLevel(row, row.level) } : null;
}
...
export function getMutationGroups(tgId) {
  const rows = db.prepare(`
    SELECT uc.card_id, uc.level, COUNT(*) AS cnt, c.name, c.image_url, c.level_images, c.rarity, c.base_power, c.max_level
    FROM user_cards uc JOIN game_cards c ON c.id = uc.card_id
    WHERE uc.tg_id = ?
...
// یکی حذف می‌شه و اون یکی یه سطح میره بالا
